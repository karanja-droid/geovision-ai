...
**Status**: Phase 0 Foundation **COMPLETE** (2026-06-28). Runnable MVP demo live on the `setup/foundation` branch / PR #13. Full code delivered (Docker, FastAPI + SQLModel + PostGIS, Streamlit with interactive map + exports, basic models, Copperbelt demo data). See PR for details.
...
**Next update: after Phase 1 (data-foundation) completion**

---

**Phase 0 delivered (see PR #13)**:
- Full recommended folder structure + key files (pyproject.toml with uv, docker-compose.yml with postgres-postgis/backend/streamlit, backend/app/main.py + models + services, Streamlit app.py with full MVP flow)
- GitHub setup: labels, initial issues + milestone references, PR template, .github/workflows/ci.yml
- Initial README + this updated DEVELOPMENT_LOOP.md
- Basic .gitignore, LICENSE (MIT), CODEOWNERS
- Working local environment: `docker compose up --build` gives runnable demo of the MVP success criteria.
- Copperbelt demo data + simple prospectivity (adaptable to real training).

See README.md for quickstart and open issues for next inner loops (Phase 1: full data layer, AOI ingestion, real DB CRUD, spatial queries, etc.).