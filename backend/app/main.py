...
async def lifespan(app: FastAPI):
    logger.info("🚀 Starting GeoVision AI backend (Phase 0 + Phase 1 data layer + weather integration)")
...
async def health():
    ...
    return {
        ...
        "phase": "0-foundation + phase1-data + weather"
    }

@app.get("/", tags=["system"])
async def root():
    return {
        "message": "Welcome to GeoVision AI (Phase 0+1 + weather). See /docs and docs/DEVELOPMENT_LOOP.md",
        "docs": "/docs",
        "health": "/health"
    }
...
app.include_router(weather_router, prefix="/api/v1", tags=["weather", "real-time"])
